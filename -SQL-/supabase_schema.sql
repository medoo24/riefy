...
DROP POLICY IF EXISTS "Auth insert sentences" ON public.sentences;
...
DROP POLICY IF EXISTS "Own vocab insert" ON public.vocabulary_bank;
CREATE POLICY "Own vocab insert"     ON public.vocabulary_bank FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Own vocab update" ON public.vocabulary_bank;
